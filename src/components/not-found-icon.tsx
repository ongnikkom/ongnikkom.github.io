interface NotFoundIconsProps extends React.SVGProps<SVGSVGElement> {}

export default function NotFoundIcon(props: NotFoundIconsProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={514}
      height={164}
      fill="none"
      {...props}
    >
      <circle cx={101} cy={22} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={101} cy={142} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={21} cy={102} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={141} cy={102} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={193} cy={82} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={313} cy={82} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={253} cy={22} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={253} cy={142} r={20} stroke="#667085" strokeWidth={2} />
      <path
        stroke="#667085"
        strokeWidth={2}
        d="M1 102c0-11.046 8.954-20 20-20h120c11.046 0 20 8.954 20 20s-8.954 20-20 20H21c-11.046 0-20-8.954-20-20Z"
      />
      <path
        stroke="#667085"
        strokeWidth={2}
        d="M101 162c-11.046 0-20-8.954-20-20V22c0-11.046 8.954-20 20-20s20 8.954 20 20v120c0 11.046-8.954 20-20 20Z"
      />
      <path
        stroke="#667085"
        strokeWidth={2}
        d="M7.142 115.995c-7.81-7.811-7.81-20.474 0-28.284L86.711 8.142c7.81-7.81 20.473-7.81 28.284 0 7.81 7.81 7.81 20.474 0 28.284l-79.569 79.569c-7.81 7.81-20.473 7.81-28.284 0Z"
      />
      <circle cx={453} cy={22} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={453} cy={142} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={373} cy={102} r={20} stroke="#667085" strokeWidth={2} />
      <circle cx={493} cy={102} r={20} stroke="#667085" strokeWidth={2} />
      <path
        stroke="#667085"
        strokeWidth={2}
        d="M353 102c0-11.046 8.954-20 20-20h120c11.046 0 20 8.954 20 20s-8.954 20-20 20H373c-11.046 0-20-8.954-20-20Z"
      />
      <path
        stroke="#667085"
        strokeWidth={2}
        d="M453 162c-11.046 0-20-8.954-20-20V22c0-11.046 8.954-20 20-20s20 8.954 20 20v120c0 11.046-8.954 20-20 20Z"
      />
      <path
        stroke="#667085"
        strokeWidth={2}
        d="M359.142 115.995c-7.81-7.811-7.81-20.474 0-28.284l79.569-79.569c7.81-7.81 20.473-7.81 28.284 0 7.81 7.81 7.81 20.474 0 28.284l-79.569 79.569c-7.81 7.81-20.473 7.81-28.284 0Z"
      />
      <circle cx={253} cy={82} r={80} stroke="#667085" strokeWidth={2} />
      <circle cx={253} cy={82} r={40} stroke="#667085" strokeWidth={2} />
      <path stroke="#667085" strokeWidth={2} d="M0 1h513M0 163h513" />
    </svg>
  );
}
